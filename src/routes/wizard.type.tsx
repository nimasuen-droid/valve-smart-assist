...
function TypeStep() {
  ...
  const isOverridden =
    !!input.valveTypeOverride && input.valveTypeOverride !== engineResult.valveType;
  ...
  return (
    <StepShell
      ...
      aside={
        <>
          <WhyCard>
            {r?.reason || "Selection rationale will appear once inputs are complete."}
          </WhyCard>
          <LearningMoment>
            Valve type is driven by <strong>function first</strong> (isolation, throttling, check,
            relief), then refined by service, size and class. For ball valves,{" "}
            <strong>Reduced Bore is the cost-driven default</strong>; specify Full Bore only when an
            engineering requirement (piggable, ESD/HIPPS, subsea, low-dP) applies.
          </LearningMoment>
          {r?.refs?.map((ref) => (
    ...
    >
      ...
      {(() => {
        const isCritical =
          !!asmeWarning && (asmeWarning.type === "pressure" || asmeWarning.type === "material");
              ...
              {asmeRec && (
                <div className="rounded-md border border-success/30 bg-success/5 p-2.5">
                  ...
                  <p className="mt-1 pl-5 text-[11px] text-muted-foreground">
                    Based on input conditions and engineering logic — {asmeRec.note}
                  </p>
                </div>
              )}
                      ...
                      <SelectContent>
                        {(PRESSURE_CLASSES as string[]).map((c) => (
                          <SelectItem key={c} value={c}>
                            {asmeRec && c === asmeRec.recommendedClass ? (
                  ...
                  <div className="flex items-center justify-end gap-2 pt-1">
                    {asmeRec && classMismatch && (
                      <Button
                        ...
                        className="h-7 text-xs"
                        onClick={() => {
                          update({ pressureClass: asmeRec.recommendedClass });
                          setReason("pressureClass", "");
                          setClassUnlocked(false);
                        }}
                      >
        ...
        <Card>
          <CardContent className="space-y-3 p-4">
            ...
            <div className="rounded-md border border-success/30 bg-success/5 p-2.5">
              ...
              <p className="mt-1 pl-5 text-[11px] text-muted-foreground">
                Based on input conditions and engineering logic — cost-optimised default for ball
                valves.
              </p>
            </div>
            ...
            {!boreUnlocked && !input.boreOverride ? (
              ...
              <div className="space-y-2 rounded-md border border-warning/30 bg-warning/5 p-3">
                <p className="text-[11px] text-warning">
                  Switch to Full Bore only when piggable, ESD/HIPPS, subsea, or pressure-drop
                  requirements apply.
                </p>
                ...
                <div>
                  <label className="mb-1 block text-[11px] font-medium uppercase tracking-wider text-muted-foreground">
                    Override reason{" "}
                    {input.boreOverride && <span className="text-destructive">*</span>}
                  </label>
                  <Textarea
                    value={boreReason}
                ...
                <div className="flex items-center justify-end pt-1">
                  <Button
                    ...
                    onClick={() => {
                      update({ boreOverride: "" });
                      setReason("boreOverride", "");
                      setBoreUnlocked(false);
                    }}
                  >
                    <Lock className="h-3 w-3" /> Use recommended
                  </Button>
