...
const TYPICAL_CV: Record<string, Record<string, number>> = {
  "Globe Valve":     { '1"': 12, '1.5"': 28, '2"': 48, '3"': 110, '4"': 195, '6"': 450, '8"': 750, '10"': 1100, '12"': 1600 },
  "Ball Valve":      { '1"': 50, '1.5"': 130, '2"': 230, '3"': 580, '4"': 1100, '6"': 2400, '8"': 4200, '10"': 6500, '12"': 9500 },
  "Butterfly Valve": { '1"': 30, '1.5"': 80, '2"': 150, '3"': 400, '4"': 800, '6"': 1900, '8"': 3500, '10"': 5500, '12"': 8200, '14"': 11000, '16"': 15000, '18"': 19000, '20"': 24000, '24"': 35000 },
};
...
export function runSizing(inp: SizingInputs): SizingResult {
  ...
  if (!isFinite(inp.inletPressureBarg) || inp.inletPressureBarg < 0) errors.push("Inlet pressure missing or invalid.");
  if (!isFinite(inp.pressureDropBar) || inp.pressureDropBar <= 0) errors.push("Pressure drop must be > 0.");
  ...
  if (errors.length) {
    return { ok: false, errors, requiredCv: 0, requiredKv: 0, choked: false, verdict: "N/A", verdictNote: "Inputs incomplete.", assumedXt: xt, references: refs };
  }
  ...
  if (inp.phase === "liquid") {
    ...
    if (!inp.specificGravity || inp.specificGravity <= 0) errors.push("Specific gravity is required.");
...
export function evaluateAgainstValve(
  ...
  pipeSize: string,
): { verdict: SizingResult["verdict"]; verdictNote: string; typicalCv?: number; openingPct?: number } {
  if (!required.ok) return { verdict: "N/A", verdictNote: "Sizing not computed." };
  if (!valveType || !TYPICAL_CV[valveType]) {
    return { verdict: "N/A", verdictNote: `No Cv reference for ${valveType ?? "(unknown)"} — manual vendor check required.` };
  }
  const typicalCv = TYPICAL_CV[valveType][pipeSize];
  if (!typicalCv) {
    return { verdict: "N/A", verdictNote: `No Cv reference for ${valveType} ${pipeSize} — manual vendor check required.` };
  }
  const opening = (required.requiredCv / typicalCv) * 100;
  if (opening > 100) {
    return { verdict: "UNDERSIZED", verdictNote: `Required Cv ${required.requiredCv.toFixed(1)} exceeds typical full-open Cv (${typicalCv}) for ${valveType} ${pipeSize}. Increase line size or change valve type.`, typicalCv, openingPct: opening };
  }
  if (opening < 20) {
    return { verdict: "REVIEW", verdictNote: `Valve operates at ${opening.toFixed(0)}% open — risk of poor control / seat erosion. Consider smaller body or trim.`, typicalCv, openingPct: opening };
  }
  if (opening > 80) {
    return { verdict: "REVIEW", verdictNote: `Valve operates at ${opening.toFixed(0)}% open — little margin for upset / future capacity. Review trim selection.`, typicalCv, openingPct: opening };
  }
  return { verdict: "PASS", verdictNote: `Required Cv ${required.requiredCv.toFixed(1)} vs typical full-open Cv ${typicalCv} → ~${opening.toFixed(0)}% open. Within recommended 20–80% control window.`, typicalCv, openingPct: opening };
}
